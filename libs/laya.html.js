(function (exports, Laya) {
    ...
    class HTMLStyle {
        ...
        get font() {
            return (this.italic ? "italic " : "") + (this.bold ? "bold " : "") + this.fontSize + "px " + (Laya.ILaya.Browser.onIPhone ? (Laya.Config.fontFamilyMap[this.family] || this.family) : this.family);
        }
        set block(value) {
        ...
        reset() {
            ...
            this._type = 0;
            this.wordWrap = true;
            this.fontSize = Laya.Config.defaultFontSize;
            this.family = Laya.Config.defaultFont;
            this.color = "#000000";
    ...
    class HTMLLinkElement extends HTMLElement {
        ...
        set href(url) {
            ...
            Laya.ILaya.loader.fetch(url, "text").then(data => {
                switch (this.type) {
                    case 'text/css':
    ...
    class HTMLImageElement extends HTMLElement {
        ...
        set src(url) {
            url = Laya.URL.formatURL(url, this.URI ? this.URI.path : null);
            if (this._url === url)
                return;
            this._url = url;
            Laya.ILaya.loader.load(url).then((tex) => {
                this._tex = tex;
                this.onloaded();
            });
        }
    ...
    class HTMLIframeElement extends HTMLDivElement {
        ...
        set href(url) {
            url = Laya.URL.formatURL(url, this._element.URI ? this._element.URI.path : null);
            Laya.ILaya.loader.fetch(url, "text").then((data) => {
