...
export class TextRender {
    ...
    _fast_filltext(ctx, data, x, y, font, color, strokeColor, lineWidth, textAlign) {
        ...
        var ri = null;
        var splitTex = this.renderPerChar = (!isWT) || TextRender.forceSplitRender || (isWT && wt.splitRender);
    ...
    _drawResortedWords(ctx, startx, starty, samePagesData) {
        ...
        for (var id in samePagesData) {
            ...
            for (var j = 0; j < pisz; j++) {
                ...
                if (LayaEnv.isConch) {
                    ctx._drawTextureM(tex.texture, startx + riSaved.x - ri.orix, starty + riSaved.y - ri.oriy, riSaved.w, riSaved.h, null, 1.0, ri.uv, 0xffffffff);
                }
                else {
                    let t = tex;
                    ctx._inner_drawTexture(t.texture, t.id, startx + riSaved.x - ri.orix, starty + riSaved.y - ri.oriy, riSaved.w, riSaved.h, mat, ri.uv, 1.0, isLastRender, 0xffffffff);
                }
                if (ctx.touches) {
                    ctx.touches.push(ri);
                }
    ...
    filltext_native(ctx, data, x, y, fontStr, color, strokeColor, lineWidth, textAlign) {
