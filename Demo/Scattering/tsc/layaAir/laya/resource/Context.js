...
export class Context {
    ...
    _fillTexture(texture, texw, texh, texuvRect, x, y, width, height, type, offsetx, offsety, color) {
        ...
        uv[7] = edv;
        if (!this.clipedOff(this._transedPoints)) {
            var rgba = this._mixRGBandAlpha(color, this._shader2D.ALPHA);
    ...
    _inner_drawTexture(tex, imgid, x, y, width, height, m, uv, alpha, lastRender, color) {
        ...
        var rgba = this._mixRGBandAlpha(color, this._shader2D.ALPHA * alpha);
        if (lastRender) {
    ...
    drawTarget(rt, x, y, width, height, m, shaderValue, uv = null, blend = -1, color = 0xffffffff) {
        this._drawCount++;
        if (this._mesh.vertNum + 4 > Context._MAXVERTNUM) {
            this._mesh = MeshQuadTexture.getAMesh(this.isMain);
            this.meshlist.push(this._mesh);
