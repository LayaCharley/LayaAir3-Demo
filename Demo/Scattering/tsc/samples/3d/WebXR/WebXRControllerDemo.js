...
export class WebXRControllerDemo {
    ...
    bindMeshRender(webXRInput) {
        let rightControl = Laya.loader.getRes("res/OculusController/controller.gltf");
        let leftControl = Laya.loader.getRes("res/OculusController/controller-left.gltf");
    ...
    getRightInput(rightInput) {
        ...
        // 射线方向坐标计算
        rightInput.ray.origin.x + Vector3.dot(rightInput.ray.direction, new Vector3(1, 0, 0)) / directionMod * this.rightTargetDistance, rightInput.ray.origin.y + Vector3.dot(rightInput.ray.direction, new Vector3(0, 1, 0)) / directionMod * this.rightTargetDistance, rightInput.ray.origin.z + Vector3.dot(rightInput.ray.direction, new Vector3(0, 0, 1)) / directionMod * this.rightTargetDistance);
        // 碰撞结果
    ...
    getLeftInput(leftInput) {
        ...
        var endPos = new Vector3(
        // 射线方向坐标计算
        leftInput.ray.origin.x + Vector3.dot(leftInput.ray.direction, new Vector3(1, 0, 0)) / directionMod * this.leftTargetDistance, leftInput.ray.origin.y + Vector3.dot(leftInput.ray.direction, new Vector3(0, 1, 0)) / directionMod * this.leftTargetDistance, leftInput.ray.origin.z + Vector3.dot(leftInput.ray.direction, new Vector3(0, 0, 1)) / directionMod * this.leftTargetDistance);
        // 碰撞结果
        var hitRes = new HitResult();
