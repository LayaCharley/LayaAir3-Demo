...
export class PhysicsWorld_RayShapeCast {
    constructor() {
        ...
        this._albedoColor = new Color(1.0, 1.0, 1.0, 0.5);
    ...
    loadUI() {
        Laya.loader.load(["res/threeDimen/ui/button.png"], Handler.create(this, function () {
            this.changeActionButton0 = Laya.stage.addChild(new Button("res/threeDimen/ui/button.png", "射线模式"));
            ...
            this.changeActionButton1 = Laya.stage.addChild(new Button("res/threeDimen/ui/button.png", "不穿透"));
            ...
            this.changeActionButton2 = Laya.stage.addChild(new Button("res/threeDimen/ui/button.png", "检测"));
    ...
    stypeFun2(castType = 0) {
        if (this.hitResult.succeeded)
            this.hitResult.collider.owner.meshRenderer.sharedMaterial.albedoColor = new Color(1.0, 1.0, 1.0, 1.0);
        if (this.hitResults.length > 0) {
            for (var i = 0, n = this.hitResults.length; i < n; i++)
                this.hitResults[i].collider.owner.meshRenderer.sharedMaterial.albedoColor = new Color(1.0, 1.0, 1.0, 1.0);
            ...
            case 0:
                ...
                if (this.castAll) {
                    ...
                    for (i = 0, n = this.hitResults.length; i < n; i++)
                        //将射线碰撞到的物体设置为红色
                        this.hitResults[i].collider.owner.meshRenderer.sharedMaterial.albedoColor = new Color(1.0, 0.0, 0.0, 1.0);
                }
                else {
                    //进行射线检测,检测第一个碰撞物体
                    this.scene.physicsSimulation.raycastFromTo(this.from, this.to, this.hitResult);
                    //将检测到的物体设置为红色
                    this.hitResult.collider.owner.meshRenderer.sharedMaterial.albedoColor = new Color(1.0, 0.0, 0.0, 1.0);
                }
                break;
            case 1:
                //创建盒型碰撞器
