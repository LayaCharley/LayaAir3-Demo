...
class TriggerCollisionScript extends Script {
    ...
    onTriggerEnter(other) {
        this.owner.meshRenderer.sharedMaterial.albedoColor = new Color(0.0, 1.0, 0.0, 1.0);
    ...
    //结束触发时执行
    onTriggerExit(other) {
        this.owner.meshRenderer.sharedMaterial.albedoColor = new Color(1.0, 1.0, 1.0, 1.0);
        console.log("onTriggerExit");
    }
    //开始碰撞时执行
    onCollisionEnter(collision) {
        if (collision.other.owner === this.kinematicSprite)
            this.owner.meshRenderer.sharedMaterial.albedoColor = new Color(0.0, 0.0, 0.0, 1.0);
    }
    //持续碰撞时执行
