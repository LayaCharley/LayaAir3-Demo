...
export class DynamicBatchTest {
    constructor() {
        Laya.init(0, 0).then(() => {
            ...
            Texture2D.load("res/threeDimen/layabox.png", Handler.create(null, function (tex) {
                var radius = new Vector3(0, 0, 1);
                var radMatrix = new Matrix4x4();
                var circleCount = 50;
                var boxMesh = PrimitiveMesh.createBox(0.02, 0.02, 0.02);
                var boxMat = new BlinnPhongMaterial();
                ...
                for (var i = 0; i < circleCount; i++) {
                    ...
                    for (var j = 0; j < oneCircleCount; j++) {
                        var boxSprite = new MeshSprite3D(boxMesh);
