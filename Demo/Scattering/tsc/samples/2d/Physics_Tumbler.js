...
export class Physics_Tumbler {
    ...
    createBox() {
        ...
        const posx = Browser.width / 2, posy = Browser.height / 2;
        ...
        let boxBody = box.addComponent(RigidBody);
        // boxBody.gravityScale = 0;
        let box1Shape = box.addComponent(BoxCollider);
        let box2Shape = box.addComponent(BoxCollider);
        let box3Shape = box.addComponent(BoxCollider);
        let box4Shape = box.addComponent(BoxCollider);
    ...
    eventListener() {
        let label = this.label = Laya.stage.addChild(new Label("双击屏幕，将会产生100个新的小刚体"));
        label.top = 20;
        label.right = 20;
        label.fontSize = 16;
        label.color = "#e69999";
