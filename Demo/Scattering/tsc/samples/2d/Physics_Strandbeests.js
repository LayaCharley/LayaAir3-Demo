...
export class Physics_Strandbeests {
    ...
    Construct() {
        ...
        for (let i = 1; i <= 32; i++) {
            ...
            let sBody = small.addComponent(RigidBody);
            let sCollider = small.addComponent(CircleCollider);
            sCollider.radius = 2.5 * this.scale;
        }
        // Chassis
        let chassis = this.chassis = new Sprite();
        ...
        let chassisBody = chassis.addComponent(RigidBody);
        chassisBody.group = -1;
        let chassisCollider = chassis.addComponent(BoxCollider);
        ...
        let wheelBody = wheel.addComponent(RigidBody);
        wheelBody.group = -1;
        let wheelCollider = wheel.addComponent(CircleCollider);
        wheelCollider.density = 1;
        wheelCollider.radius = 16 * this.scale;
        wheelCollider.x = -16 * this.scale;
        wheelCollider.y = -16 * this.scale;
        // 转动关节
        let motorJoint = this.motorJoint = new RevoluteJoint();
    ...
    createLeg(s, wheelAnchor) {
        ...
        const wheelBody = this.wheel.getComponent(RigidBody);
        ...
        let legBody1 = leg1.addComponent(RigidBody);
        legBody1.angularDamping = 10;
        legBody1.group = -1;
        let legCollider1 = leg1.addComponent(PolygonCollider);
        ...
        let legBody2 = leg2.addComponent(RigidBody);
        legBody2.angularDamping = 10;
        legBody2.group = -1;
        let legCollider2 = leg2.addComponent(PolygonCollider);
    ...
    eventListener() {
        ...
        let label = this.label = Laya.stage.addChild(new Label("双击屏幕，仿生机器人向相反方向运动\n单击产生新的小球刚体"));
        label.top = 20;
        label.right = 20;
        label.fontSize = 16;
        label.color = "#e69999";
